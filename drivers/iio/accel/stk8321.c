...
#include <linux/interrupt.h>   // irqreturn_t, IRQ_HANDLED, IRQF_*
#include <linux/gpio/consumer.h> // gpiod_get(), devm_gpiod_get(), etc.
#include <linux/of_gpio.h>     // of_get_named_gpio (deprecated, but you used it)
#include <linux/pm.h>          // PM ops
#include <linux/i2c.h>
...
static int stk8321_apply_orientation(struct iio_dev *indio_dev)
{
    ...
    // Then you can do something here
...
//	const struct i2c_device_id *id = i2c_client_get_device_id(client);
	...
	data = iio_priv(indio_dev);
        ...
        // Interrupts request
        irq_gpio = of_get_named_gpio(client->dev.of_node, "interrupt-gpios", 0);
        if (!gpio_is_valid(irq_gpio)){
	    dev_err(&client->dev, "gpio get by name failed!\n");
            return -EINVAL;
        }
        

        ret = devm_gpio_request_one(&client->dev, irq_gpio, GPIOF_IN, "stk8321_int");
        if (ret)
            return ret;
	...
	indio_dev->num_channels = ARRAY_SIZE(stk8321_channels);
        ...
        ret = iio_read_mount_matrix(dev, "mount-matrix", &data->orientation);
        if (ret < 0){
            pr_warn("No mount-matrix DT property found, using default\n");
            data->orientation = iio_mount_zeromatrix;
        }
...
//				stk8321_resume);
static SIMPLE_DEV_PM_OPS(stk8321_pm_ops,
                         stk8321_suspend,
                         stk8321_resume);

#define STK8321_PM_OPS (&stk8321_pm_ops)
#else
#define STK8321_PM_OPS NULL
#endif
...
static const struct of_device_id stk8321_of_match[] = {
    { .compatible = "sensortek,stk8321" },
    { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, stk8321_of_match);

static struct i2c_driver stk8321_driver = {
	.driver = {
		.name = STK8321_DRIVER_NAME,
		.pm = STK8321_PM_OPS,
                .of_match_table = of_match_ptr(stk8321_of_match),
	},
	.probe =            stk8321_probe,
...
MODULE_AUTHOR("Lysander Lyu <lvyc@realbom.com>");
